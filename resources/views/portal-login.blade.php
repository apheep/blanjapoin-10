...
<body class="min-h-screen bg-gradient-to-b from-gray-50 via-white to-purple-50 font-poppins">
        ...
        <div class="w-full max-w-md">
            <div class="bg-white rounded-3xl shadow-2xl border border-purple-100 overflow-hidden">
                <div class="px-8 pt-8 pb-4 text-center">
                    ...
                    <p class="text-sm text-neutral-500 mt-1">Login menggunakan nomor WhatsApp PIC</p>
                </div>
                ...
                @if(session('success'))
                    <div class="mx-6 mb-4 rounded-2xl bg-green-50 border border-green-100 text-green-700 px-4 py-3 text-sm">
                        {{ session('success') }}
                    </div>
                @endif
                ...
                <div class="px-6 pb-6">
                    <div class="space-y-4">
                        <div class="relative">
                            <label for="wa_pic" class="block text-xs font-semibold text-neutral-700 mb-1">Nomor WhatsApp PIC</label>
                            <div class="relative">
                                <div class="absolute left-3 top-2.5 text-neutral-500 text-sm font-medium z-10">62</div>
                                <input type="text" id="wa_pic" name="wa_pic" placeholder="8xxxxxxxxxx" value="{{ old('wa_pic', $waPic ?? '') }}" required autofocus class="w-full rounded-xl border border-neutral-300 bg-white px-4 py-2 pl-12 text-sm outline-none ring-0 focus:border-purple-400 focus:ring-2 focus:ring-purple-400">
                            </div>
                            @error('wa_pic')
                        ...
                        <div class="relative">
                            ...
                            <div class="grid grid-cols-3 gap-2">
                                <label class="flex items-center p-3 rounded-xl border border-neutral-300 cursor-pointer hover:border-green-400 hover:bg-green-50 transition-all has-[:checked]:border-green-500 has-[:checked]:bg-green-50">
                                    <input type="radio" name="otp_type" value="whatsapp" class="sr-only peer" {{ (!old('otp_type') && !session('portal_otp_type')) || old('otp_type', session('portal_otp_type')) == 'whatsapp' ? 'checked' : '' }}>
                                ...
                                <label class="flex items-center p-3 rounded-xl border border-neutral-300 cursor-pointer hover:border-blue-400 hover:bg-blue-50 transition-all has-[:checked]:border-blue-500 has-[:checked]:bg-blue-50">
                                    <input type="radio" name="otp_type" value="telegram" class="sr-only peer" {{ old('otp_type', session('portal_otp_type')) == 'telegram' ? 'checked' : '' }}>
                                    <div class="flex items-center gap-2 w-full">
                                ...
                                <label class="flex items-center p-3 rounded-xl border border-neutral-300 cursor-pointer hover:border-orange-400 hover:bg-orange-50 transition-all has-[:checked]:border-orange-500 has-[:checked]:bg-orange-50">
                                    <input type="radio" name="otp_type" value="emailphone" class="sr-only peer" {{ old('otp_type', session('portal_otp_type')) == 'emailphone' ? 'checked' : '' }}>
                                    <div class="flex items-center gap-2 w-full">
                        ...
                        <form method="POST" action="{{ route('portal.send-otp') }}" id="sendOtpForm" class="mb-4">
                            @csrf
                            <input type="hidden" name="wa_pic" id="wa_pic_hidden" value="">
                            <input type="hidden" name="otp_type" id="otp_type_hidden" value="">
                            <button type="submit" id="sendOtpBtn" class="w-full rounded-xl px-4 py-2.5 text-sm font-semibold text-white shadow-lg bg-gradient-to-r from-purple-500 to-purple-600 ring-1 ring-white/30 transition-all hover:shadow-xl hover:scale-105 active:scale-95">Kirim OTP</button>
                        </form>

                        @if(session('otp_redirect_url') && (session('portal_otp_type') == 'whatsapp' || session('portal_otp_type') == 'telegram'))
                        <div class="rounded-xl bg-blue-50 border border-blue-200 p-4 mb-4">
                            <p class="text-xs text-blue-700 mb-2">OTP telah dikirim via {{ session('portal_otp_type') == 'whatsapp' ? 'WhatsApp' : 'Telegram' }}. Membuka aplikasi...</p>
                        </div>
                        @endif

                        <form method="POST" action="{{ route('portal.authenticate') }}" class="space-y-4" id="loginForm">
                            @csrf
                            <input type="hidden" name="wa_pic" value="{{ old('wa_pic', session('portal_otp_phone_display')) }}">

                            <div class="relative">
                                <label for="otp" class="block text-xs font-semibold text-neutral-700 mb-1">Kode OTP</label>
                                <div class="relative">
                                    <input type="text" id="otp" name="otp" placeholder="Masukkan 6 digit OTP" maxlength="6" pattern="[0-9]{6}" value="{{ old('otp') }}" required class="w-full rounded-xl border border-neutral-300 bg-white px-4 py-2 pl-10 text-sm outline-none ring-0 focus:border-purple-400 focus:ring-2 focus:ring-purple-400">
                            ...
                            <button type="submit" class="mb-2 mt-2 w-full rounded-xl px-4 py-2.5 text-sm font-semibold text-white shadow-lg bg-gradient-to-r from-purple-500 via-purple-600 to-purple-700 ring-1 ring-white/30 transition-all hover:shadow-xl hover:scale-105 active:scale-95">Login</button>
                        </form>
                    </div>
                </div>
            </div>
    ...
    @php
        $shouldAutoRedirect = session('portal_otp_redirect_url') && (session('portal_otp_type') == 'whatsapp' || session('portal_otp_type') == 'telegram');
        $redirectUrl = session('portal_otp_redirect_url', '');
    @endphp
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const waPicInput = document.getElementById('wa_pic');
        
        // Format value yang sudah ada jika ada dari session
        if (waPicInput.value) {
            let currentValue = waPicInput.value.replace(/\D/g, '');
            if (currentValue.startsWith('62')) {
                waPicInput.value = currentValue.substring(2);
            } else if (currentValue.startsWith('0')) {
                waPicInput.value = currentValue.substring(1);
            } else {
                waPicInput.value = currentValue;
            }
        }

        // Format input saat user mengetik
        waPicInput.addEventListener('input', function() {
        ...
        // Format saat blur
        waPicInput.addEventListener('blur', function() {
            let value = this.value.replace(/\D/g, '');
        ...
        sendOtpForm.addEventListener('submit', function(e) {
            ...
            let waPic = document.getElementById('wa_pic').value.replace(/\D/g, '');
            ...
            if (waPic.startsWith('0')) {
                waPic = waPic.substring(1);
            }
            
            // Tambahkan prefix 62 jika belum ada
            if (!waPic.startsWith('62')) {
                waPic = '62' + waPic;
            }

            const otpType = otpTypeInput.value;
            
            console.log('Sending OTP:', { waPic, otpType });

            // Set hidden input values
            document.getElementById('wa_pic_hidden').value = waPic;
