<section id="section-beauty" class="mt-10 md:mt-14">
 ...
 <div class="card-container grid grid-cols-2 gap-3 lg:grid-cols-1 lg:gap-5 items-stretch">
  ...
  <article onclick="window.open('{{ $keyword->cta_link ?? '#' }}', '_blank')" class="group overflow-hidden rounded-xl md:rounded-2xl border border-neutral-200 bg-white shadow-md transition-all hover:shadow-xl hover:scale-[1.01] hover:border-pink-200 cursor-pointer opacity-0 translate-y-2 duration-200 ease-out h-full min-h-[280px]">
   <!-- Mobile Layout -->
   <div class="lg:hidden flex flex-col h-full">
    <div class="relative">
     <div class="aspect-[4/3] rounded-t-xl bg-gradient-to-br from-neutral-100 to-neutral-200 shadow-inner overflow-hidden">
      <img src="{{ $keyword->image ? asset($keyword->image) : asset('storage/promo/promo-default.jpg') }}" alt="{{ $keyword->nama_produk }}" class="w-full h-full object-cover" loading="lazy">
     </div>
    </div>
 ...
 <div id="extraBeautyCard" class="group max-h-0 overflow-y-hidden opacity-0 scale-y-0 origin-top transition-all duration-500 ease-in-out mt-6 md:mt-10">
  <div class="card-container grid grid-cols-2 gap-3 lg:grid-cols-1 lg:gap-5 items-stretch">
   @foreach($extraKeywords as $keyword)
   <article onclick="window.open('{{ $keyword->cta_link ?? '#' }}', '_blank')" class="group overflow-hidden rounded-xl md:rounded-2xl border border-neutral-200 bg-white shadow-md transition-all hover:shadow-xl hover:scale-[1.01] hover:border-pink-200 cursor-pointer h-full min-h-[280px]">
    <!-- Mobile Layout -->
    <div class="lg:hidden flex flex-col h-full">
     <div class="relative">
      <div class="aspect-[4/3] rounded-t-xl bg-gradient-to-br from-neutral-100 to-neutral-200 shadow-inner overflow-hidden">
 ...
 <div class="mt-12 md:mt-10 flex justify-center relative z-20 md:static md:z-auto pointer-events-auto">
  <button onclick="toggleBeautyCards()" id="beautySeeAllBtn" class="text-sm font-bold text-rose-600 hover:text-rose-700 transition-all flex items-center gap-2 group px-4 py-2 rounded-full hover:bg-rose-50 bg-white/80 backdrop-blur-sm shadow-sm md:bg-transparent md:backdrop-blur-0 md:shadow-none">
