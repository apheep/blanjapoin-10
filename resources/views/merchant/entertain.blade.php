<section id="section-entertain" class="mt-10 md:mt-14 mb-10 md:mb-14">
    ...
    <div id="entertainCardContainer" data-voucher-container="true" data-voucher-section="entertain" data-container-type="primary" class="card-container grid grid-cols-2 gap-3 lg:grid-cols-3 lg:gap-5 items-stretch px-1">
        @forelse($visibleKeywords as $keyword)
            ...
            <article
                ...
                class="group voucher-card overflow-hidden rounded-xl md:rounded-2xl border border-neutral-200/80 bg-white shadow-md hover:shadow-xl transition-all duration-300 hover:border-orange-300 hover:-translate-y-1 cursor-pointer opacity-0 translate-y-2 duration-200 ease-out h-full min-h-[280px] {{ $loop->iteration === 3 ? 'hidden lg:block' : '' }}"
            >
                <!-- Mobile Layout -->
    ...
    @if($extraKeywords->isNotEmpty())
        <div id="extraEntertainCard" class="group max-h-0 overflow-hidden opacity-0 scale-y-0 origin-top transition-all duration-500 ease-in-out mt-6 md:mt-10">
            <div data-voucher-container="true" data-voucher-section="entertain" data-container-type="extra" class="card-container grid grid-cols-2 gap-3 lg:grid-cols-3 lg:gap-5 items-stretch px-1">
                @foreach($extraKeywords as $keyword)
                    @php
                        ...
                        $searchName = strtolower(trim($merchantName . ' ' . $productName));
                        $searchLocation = strtolower($locationName);
                        $uniqueId = 'entertain-card-extra-' . $keyword->id;
                    @endphp
                    <article
                        data-voucher-card="true"
                        data-search-name="{{ $searchName }}"
    ...
    <div class="mt-12 md:mt-10 flex justify-center">
        <button
            ...
            class="relative inline-flex items-center justify-center gap-2 px-6 py-3 text-sm font-bold text-rose-600 bg-white border-2 border-rose-200 rounded-full shadow-md hover:shadow-lg hover:bg-rose-50 hover:border-rose-300 transition-all duration-300 group"
        >
            <span id="entertainSeeAllText">See All</span>
            <span id="entertainSeeAllArrow" class="inline-flex items-center justify-center w-5 h-5 rounded-full bg-rose-100 group-hover:bg-rose-200 transition-all duration-300 group-hover:translate-y-1">
    ...
    <script>
        ...
        function checkTruncatedTextEntertain() {
        ...
        const originalToggleEntertainCards = window.toggleEntertainCards;
        window.toggleEntertainCards = function() {
            if (typeof originalToggleEntertainCards === 'function') {
                originalToggleEntertainCards();
            }

            setTimeout(function() {
                checkTruncatedTextEntertain();
            }, 500);
        };
        ...
        let resizeTimerEntertain;
        window.addEventListener('resize', function() {
            clearTimeout(resizeTimerEntertain);
            resizeTimerEntertain = setTimeout(function() {
                checkTruncatedTextEntertain();
            }, 250);
        });
    </script>
