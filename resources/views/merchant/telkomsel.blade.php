<section id="section-telkomsel" class="mt-10 md:mt-14">
 ...
 @if($extraKeywords->isNotEmpty())
  <div id="extraTelkomselCard" class="group max-h-0 overflow-hidden opacity-0 scale-y-0 origin-top transition-all duration-500 ease-in-out mt-6 md:mt-10">
   ...
   @php
    ...
    $searchName = strtolower(trim($merchantName . ' ' . $productName));
    $searchLocation = strtolower($locationName);
    $uniqueId = 'telkomsel-card-extra-' . $keyword->id;
   @endphp
 ...
 <div class="mt-12 md:mt-10 flex justify-center relative z-20 md:static md:z-auto pointer-events-auto">
  <button onclick="toggleTelkomselCards()" id="telkomselSeeAllBtn" class="relative inline-flex items-center justify-center gap-2 px-6 py-3 text-sm font-bold text-rose-600 bg-white border-2 border-rose-200 rounded-full shadow-md hover:shadow-lg hover:bg-rose-50 hover:border-rose-300 transition-all duration-300 group">
   <span id="telkomselSeeAllText">See All</span>
   <span id="telkomselSeeAllArrow" class="inline-flex items-center justify-center w-5 h-5 rounded-full bg-rose-100 group-hover:bg-rose-200 transition-all duration-300 group-hover:translate-y-1">
