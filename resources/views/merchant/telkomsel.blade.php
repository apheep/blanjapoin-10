<section id="section-telkomsel" class="mt-10 md:mt-14">
 ...
 @php
  ...
  $telkomselKeywords = $keywords->filter(function ($keyword) use ($telkomselCategory, $skipMerchantValidation) {
 ...
 <div id="telkomselCardContainer" data-voucher-container="true" data-voucher-section="telkomsel" data-container-type="primary" class="card-container grid grid-cols-2 gap-3 lg:grid-cols-3 lg:gap-5 items-stretch px-1">
  @forelse($telkomselKeywords as $keyword)
  @php
     ...
     <button disabled class="mt-1.5 w-auto inline-flex items-center justify-center bg-gray-400 text-white font-bold py-1 px-2.5 rounded-lg cursor-not-allowed text-[9px]">
      Voucher Habis
         ...
         <a href="{{ route('track.redirect', ['merchantId' => $keyword->merchant_key, 'keywordId' => $keyword->keyword_id]) }}" 
            target="_blank" data-redeem-btn data-lock-longlat="{{ $keyword->is_lock_longlat ? '1' : '0' }}"
 ...
 <!-- See More Button -->
 @if($telkomselKeywords->count() > 4)
 <div id="telkomselSeeMoreWrapper" class="flex justify-center mt-4 md:mt-5">
  <button id="telkomselSeeMoreBtn" onclick="telkomselLoadMore()" class="group inline-flex items-center gap-1.5 text-sm text-neutral-500 hover:text-orange-500 font-medium transition-colors duration-200">
   <span id="telkomselSeeMoreText">See more</span>
...
(function() {
 ...
 function batch() { return window.innerWidth < BP ? MOBILE_BATCH : DESKTOP_BATCH; }
 function cards() {
  const c = document.getElementById('telkomselCardContainer');
  return c ? Array.from(c.querySelectorAll('[data-voucher-card="true"]')) : [];
 }

 function init() {
  const all = cards();
  if (!all.length) return;
  visible = Math.min(batch(), all.length);
  all.forEach((c, i) => {
   if (i < visible) { c.style.display = ''; c.classList.remove('opacity-0','translate-y-2'); c.classList.add('opacity-100','translate-y-0'); }
   else { c.style.display = 'none'; c.classList.add('opacity-0','translate-y-2'); c.classList.remove('opacity-100','translate-y-0'); }
  });
  updateBtn(all.length);
 }

 function updateBtn(total) {
  const w = document.getElementById('telkomselSeeMoreWrapper');
  if (!w) return;
  w.style.display = visible >= total ? 'none' : 'flex';
 }

 window.telkomselLoadMore = function() {
  const all = cards(), b = batch(), end = Math.min(visible + b, all.length);
  for (let i = visible; i < end; i++) {
   const c = all[i]; c.style.display = ''; c.style.transitionDelay = ((i - visible) * 70) + 'ms';
   void c.offsetHeight;
   requestAnimationFrame(() => { c.classList.remove('opacity-0','translate-y-2'); c.classList.add('opacity-100','translate-y-0'); });
  }
  setTimeout(() => { for (let i = visible; i < end; i++) all[i].style.transitionDelay = ''; }, (end - visible) * 70 + 350);
  visible = end;
  updateBtn(all.length);
  if (all[visible - b]) setTimeout(() => all[visible - b].scrollIntoView({ behavior:'smooth', block:'nearest' }), 100);
 };

 let bp = window.innerWidth < BP ? 'm' : 'd';
 window.addEventListener('resize', () => { const n = window.innerWidth < BP ? 'm' : 'd'; if (n !== bp) { bp = n; updateBtn(cards().length); } });
...
// Telkomsel Description Bottom Sheet Function
function openTelkomselDescriptionSheet(keywordId, merchantName, productName, skb, diskon) {
 const contentHTML = `
  <div class="px-5 pb-6">
   <div class="space-y-1">
    <div>
