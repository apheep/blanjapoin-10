<section id="section-paketvideo" class="mt-10 md:mt-14 mb-10 md:mb-14">
    ...
    <!-- See More Button -->
    @if($paketvideoKeywords->count() > 4)
    <div id="paketvideoSeeMoreWrapper" class="flex justify-center mt-4 md:mt-5">
     <button onclick="paketvideoLoadMore()" class="group inline-flex items-center gap-1.5 text-sm text-neutral-500 hover:text-orange-500 font-medium transition-colors duration-200">
      <span>See more</span>
...
<script>
// === PaketVideo See More Logic ===
(function() {
 const MB = 4, DB = 6, BP = 1024; let visible = 0;
 function batch() { return window.innerWidth < BP ? MB : DB; }
 function cards() { const c = document.getElementById('paketvideoCardContainer'); return c ? Array.from(c.querySelectorAll('[data-voucher-card="true"]')) : []; }
 ...
 function updateBtn(total) { const w = document.getElementById('paketvideoSeeMoreWrapper'); if (!w) return; w.style.display = visible >= total ? 'none' : 'flex'; }
 window.paketvideoLoadMore = function() { const all = cards(), b = batch(), end = Math.min(visible + b, all.length); for (let i = visible; i < end; i++) { const c = all[i]; c.style.display = ''; c.style.transitionDelay = ((i - visible) * 70) + 'ms'; void c.offsetHeight; requestAnimationFrame(() => { c.classList.remove('opacity-0','translate-y-2'); c.classList.add('opacity-100','translate-y-0'); }); } setTimeout(() => { for (let i = visible; i < end; i++) all[i].style.transitionDelay = ''; }, (end - visible) * 70 + 350); visible = end; updateBtn(all.length); if (all[visible - b]) setTimeout(() => all[visible - b].scrollIntoView({ behavior:'smooth', block:'nearest' }), 100); };
