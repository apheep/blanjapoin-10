<section id="section-food" class="mt-10 md:mt-14 mb-10 md:mb-14">
    <div class="mb-4 md:mb-6 flex items-center justify-between">
        <h2 class="text-xl md:text-3xl font-black text-neutral-900">🍔​ Kuliner</h2>
    </div>
    ...
    <div id="foodCardContainer" data-voucher-container="true" data-voucher-section="food" data-container-type="primary" class="card-container grid grid-cols-2 gap-3 lg:grid-cols-3 lg:gap-5 items-stretch px-1">
        ...
        @empty
            <div class="col-span-10 text-center text-neutral-500 text-sm py-6">
                Belum ada data promo untuk kategori Kuliner.
            </div>
        @endforelse
    </div>

    <!-- See More Button -->
    @if($foodKeywords->count() > 4)
    <div id="foodSeeMoreWrapper" class="flex justify-center mt-4 md:mt-5">
     <button onclick="foodLoadMore()" class="group inline-flex items-center gap-1.5 text-sm text-neutral-500 hover:text-orange-500 font-medium transition-colors duration-200">
      <span>See more</span>
...
(function() {
 ...
 function cards() { const c = document.getElementById('foodCardContainer'); return c ? Array.from(c.querySelectorAll('[data-voucher-card="true"]')) : []; }
 ...
 function updateBtn(total) { const w = document.getElementById('foodSeeMoreWrapper'); if (!w) return; w.style.display = visible >= total ? 'none' : 'flex'; }
 window.foodLoadMore = function() { const all = cards(), b = batch(), end = Math.min(visible + b, all.length); for (let i = visible; i < end; i++) { const c = all[i]; c.style.display = ''; c.style.transitionDelay = ((i - visible) * 70) + 'ms'; void c.offsetHeight; requestAnimationFrame(() => { c.classList.remove('opacity-0','translate-y-2'); c.classList.add('opacity-100','translate-y-0'); }); } setTimeout(() => { for (let i = visible; i < end; i++) all[i].style.transitionDelay = ''; }, (end - visible) * 70 + 350); visible = end; updateBtn(all.length); if (all[visible - b]) setTimeout(() => all[visible - b].scrollIntoView({ behavior:'smooth', block:'nearest' }), 100); };
