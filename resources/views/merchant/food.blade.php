<section id="section-food" class="mt-10 md:mt-14 mb-10 md:mb-14">
            ...
            <div data-voucher-container="true" data-voucher-section="food" data-container-type="extra" class="card-container grid grid-cols-2 gap-3 lg:grid-cols-3 lg:gap-5 items-stretch px-1">
                @foreach($extraKeywords as $keyword)
                    @php
                        ...
                        $searchName = strtolower(trim($merchantName . ' ' . $productName));
                        $searchLocation = strtolower($locationName);
                        $uniqueId = 'food-card-extra-' . $keyword->id;
                    @endphp
    ...
    <div class="mt-12 md:mt-10 flex justify-center relative z-20 md:static md:z-auto pointer-events-auto">
        <button onclick="toggleFoodCards()" id="foodSeeAllBtn" class="relative inline-flex items-center justify-center gap-2 px-6 py-3 text-sm font-bold text-rose-600 bg-white border-2 border-rose-200 rounded-full shadow-md hover:shadow-lg hover:bg-rose-50 hover:border-rose-300 transition-all duration-300 group">
            <span id="foodSeeAllText">See All</span>
            <span id="foodSeeAllArrow" class="inline-flex items-center justify-center w-5 h-5 rounded-full bg-rose-100 group-hover:bg-rose-200 transition-all duration-300 group-hover:translate-y-1">
