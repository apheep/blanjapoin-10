<section id="section-shop" class="mt-14 md:mt-24 mb-10 md:mb-14">
     ...
     <div id="shopCardContainer" data-voucher-container="true" data-voucher-section="shop" data-container-type="primary" class="card-container grid grid-cols-2 gap-3 lg:grid-cols-3 lg:gap-5 items-stretch px-1">
      @forelse($shopKeywords as $keyword)
      @php
      ...
      @endphp
       ...
       <img src="{{ $keyword->image ? asset('storage/' . $keyword->image) : asset('storage/promo/promo-default.jpg') }}" alt="{{ $keyword->nama_produk }}" class="w-full h-full object-cover" loading="lazy">
        ...
        <div class="flex flex-col p-2.5 space-y-1 flex-1">
         ...
         <div class="text-[10px] text-neutral-600 leading-snug">
          ...
          @endif
          @if($keyword->skb)
          <button onclick="event.stopPropagation(); openShopDescriptionSheet({{ $keyword->id }}, {{ json_encode(($keyword->merchant)->nama_merchant) }}, {{ json_encode($productName) }}, {{ json_encode($keyword->skb) }}, {{ json_encode($keyword->diskon ? formatDiskon($keyword->diskon) : null) }})" class="mt-0.5 text-[9px] font-semibold text-orange-600 hover:text-orange-700 underline focus:outline-none">
           Lihat Deskripsi
          </button>
          @endif
         </div>
       ...
       <div class="hidden lg:flex flex-col h-full">
        ...
        <div class="flex flex-col px-3 md:px-4 pb-3 md:pb-4 flex-1 min-h-0">
         ...
         @endif
         @if($keyword->skb)
         <button onclick="event.stopPropagation(); openShopDescriptionSheet({{ $keyword->id }}, {{ json_encode($merchantName) }}, {{ json_encode($productName) }}, {{ json_encode($keyword->skb) }}, {{ json_encode($keyword->diskon ? formatDiskon($keyword->diskon) : null) }})" class="self-start text-left mb-1.5 text-[10px] md:text-xs font-semibold text-orange-600 hover:text-orange-700 underline focus:outline-none">
          Lihat Deskripsi
         </button>
         @endif
         @if($keyword->end_date)
     ...
     <!-- See More Button -->
     @if($shopKeywords->count() > 4)
     <div id="shopSeeMoreWrapper" class="flex justify-center mt-4 md:mt-5">
      <button onclick="shopLoadMore()" class="group inline-flex items-center gap-1.5 text-sm text-neutral-500 hover:text-orange-500 font-medium transition-colors duration-200">
       <span>See more</span>
...
(function() {
 ...
 function cards() { const c = document.getElementById('shopCardContainer'); return c ? Array.from(c.querySelectorAll('[data-voucher-card="true"]')) : []; }
 ...
 function updateBtn(total) { const w = document.getElementById('shopSeeMoreWrapper'); if (!w) return; w.style.display = visible >= total ? 'none' : 'flex'; }
 window.shopLoadMore = function() { const all = cards(), b = batch(), end = Math.min(visible + b, all.length); for (let i = visible; i < end; i++) { const c = all[i]; c.style.display = ''; c.style.transitionDelay = ((i - visible) * 70) + 'ms'; void c.offsetHeight; requestAnimationFrame(() => { c.classList.remove('opacity-0','translate-y-2'); c.classList.add('opacity-100','translate-y-0'); }); } setTimeout(() => { for (let i = visible; i < end; i++) all[i].style.transitionDelay = ''; }, (end - visible) * 70 + 350); visible = end; updateBtn(all.length); if (all[visible - b]) setTimeout(() => all[visible - b].scrollIntoView({ behavior:'smooth', block:'nearest' }), 100); };
...
// Shop Description Bottom Sheet Function
function openShopDescriptionSheet(keywordId, merchantName, productName, skb, diskon) {
 const contentHTML = `
  <div class="px-5 pb-6">
   <div class="space-y-1">
    <div>
